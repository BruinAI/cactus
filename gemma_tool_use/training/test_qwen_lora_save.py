#!/usr/bin/env python3
"""
Test script to compare LoRA model vs saved/reloaded merged Qwen3 model.
...
RANK = 32
ALPHA = 64.0
OUTPUT_DIR = "./test_output/qwen3-600m-lora-merged"
...
def run_forward_pass(model, tokenizer, input_text: str, mesh):
    ...
    input_tokens = jnp.array([token_ids] * batch_size, dtype=jnp.int32)  # [batch_size, seq_len]
...
def verify_lora_weights_changed(lora_model):
    ...
    for i in [0, len(lora_model.layers) // 2, len(lora_model.layers) - 1]:
        # Check LoRA weights
        lora_a = lora_model.layers[i].attn.q_proj.w_lora_a.value
        lora_b = lora_model.layers[i].attn.q_proj.w_lora_b.value
...
def verify_merged_weights_correct(base_model, lora_model, merged_model):
    ...
    for i in [0, len(base_model.layers) // 2, len(base_model.layers) - 1]:
        # Get base weight - for Qwen Einsum layers, the weight is stored as 'w'
        base_weight = base_model.layers[i].attn.q_proj.w.value
        ...
        actual_merged = merged_model.layers[i].attn.q_proj.w.value
...
def main():
    ...
    # Get model config
    if "Qwen3-0.6B" in MODEL_ID or "Qwen2.5-0.5B" in MODEL_ID:
