...
def _find_library():
    ...
    # Common search paths
    search_paths = [
        # Relative to this file - direct build output (most common)
        os.path.join(os.path.dirname(__file__), "..", "..", "..", "cactus", "build", f"libcactus.{_LIB_EXT}"),
        # Relative to this file - lib subdirectory
        os.path.join(os.path.dirname(__file__), "..", "..", "..", "cactus", "build", "lib", f"libcactus.{_LIB_EXT}"),
        # From current working directory - direct build
        os.path.join(os.getcwd(), "cactus", "build", f"libcactus.{_LIB_EXT}"),
        # From current working directory - lib subdirectory
...
def cactus_init(model_path: str, context_size: int) -> cactus_model_t:
    ...
    if not model:
        raise RuntimeError(f"Failed to initialize model from {os.path.abspath(model_path)}")
    return model
...
def cactus_embed(model: cactus_model_t, text: str, max_dim: int = 4096) -> list:
    ...
    result = _lib.cactus_embed(
        ...
        text.encode('utf-8'),
        embeddings_buffer,
        max_dim * ctypes.sizeof(ctypes.c_float),  # Buffer size in BYTES
        ctypes.byref(embedding_dim)
    )

    # cactus_embed returns embedding_dim (> 0) on success, <= 0 on error
    if result <= 0:
