...
bool run_whisper_test(const char* title,
    ...
    char response[1 << 15]; // 32KB JSON buffer
...
// Individual tests
// ---------------------------
static bool test_whisper_prefill_basic() {
    // Deterministic-ish decode (temperature 0), short decode
    return run_whisper_test(
        "WHISPER PREFILL + SHORT DECODE",
        /*temperature*/ 0.0f,
        /*top_p*/ 1.0f,
        /*top_k*/ 0,
        /*max_tokens*/ 16,
        /*use_streaming*/ false,
        /*stop_at*/ -1,
        [](int rc, const std::string& text, double ttft, double tps, double n_comp, int streamed_tokens) {
            bool has_text = !text.empty();
            bool some_tokens = (n_comp >= 1.0);
            return rc > 0 && has_text && some_tokens;
        }
    );
}

static bool test_whisper_autoregressive_longer() {
    // Longer decode to exercise KV-cache loop
    return run_whisper_test(
        ...
        /*stop_at*/ -1,
        [](int rc, const std::string& text, double ttft, double tps, double n_comp, int streamed_tokens) {
            bool decent_length = (n_comp >= 8.0); // expect more than a handful
            return rc > 0 && decent_length;
        }
    );
}

static bool test_whisper_streaming_early_stop() {
    // Stream tokens, stop after N tokens to test cactus_stop() path
    return run_whisper_test(
        ...
        /*stop_at*/ 20,
        [](int rc, const std::string& text, double ttft, double tps, double n_comp, int streamed_tokens) {
            // We asked to stop at 20; ensure we didn't run to full length
...
int main() {
    ...
    runner.run_test("whisper_prefill_basic",      test_whisper_prefill_basic());
