    ...
    for (size_t n = 0; n < N; ++n) {
        ...
        for (size_t out_idx = 0; out_idx < out_len; out_idx += 2) {
            ...
            for (size_t oc = 0; oc < C_out; ++oc) {
                ...
                // Vectorized over input channels in blocks of 8, like FP32
                for (; ic + 8 <= C_in; ic += 8) {
                    float x0m[8], x00[8], x0p[8];
                    float x1m[8], x10[8], x1p[8];
                    ...
                    for (size_t u = 0; u < 8; ++u) {
                        const size_t ch  = ic + u;
                        const __fp16* Xc = Xb + ch * L;

                        const ptrdiff_t tm0 = (ptrdiff_t)t0 - 1;
                        const ptrdiff_t tp0 = (ptrdiff_t)t0 + 1;

                        const __fp16 hx0m = (tm0 >= 0) ? Xc[tm0] : (__fp16)0;
                        const __fp16 hx00 =               Xc[t0];
                        const __fp16 hx0p = (tp0 < (ptrdiff_t)L) ? Xc[tp0] : (__fp16)0;
                        ...
                        if (have_t1) {
                            ...
                            const __fp16 hx1m = (tm1 >= 0) ? Xc[tm1] : (__fp16)0;
                            const __fp16 hx10 =               Xc[t1];
                            const __fp16 hx1p = (tp1 < (ptrdiff_t)L) ? Xc[tp1] : (__fp16)0;
                    ...
                    for (size_t u = 0; u < 8; ++u) {
                        const __fp16* Wc_h = Woc + (ic + u) * 3;
                        const float W0 = (float)Wc_h[0];
                        const float W1 = (float)Wc_h[1];
                        const float W2 = (float)Wc_h[2];

                        const float32x4_t wv  = { W0, W1, W2, 0.f };
                ...
                // Tail channels, scalar (still accumulating in float)
                for (; ic < C_in; ++ic) {
                    const __fp16* Xc = Xb + ic * L;
                    const __fp16* Wc_h = Woc + ic * 3;
                    ...
                    const float x0m = (float)((tm0 >= 0) ? Xc[tm0] : (__fp16)0);
                    const float x00 = (float)Xc[t0];
                    const float x0p = (float)((tp0 < (ptrdiff_t)L) ? Xc[tp0] : (__fp16)0);
                    ...
                    if (have_t1) {
                        ...
                        const float x1m = (float)((tm1 >= 0) ? Xc[tm1] : (__fp16)0);
                        const float x10 = (float)Xc[t1];
                        const float x1p = (float)((tp1 < (ptrdiff_t)L) ? Xc[tp1] : (__fp16)0);
