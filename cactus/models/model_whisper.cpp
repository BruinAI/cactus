...
void WhisperModel::load_weights_to_graph(CactusGraph* gb) {
    ...
    weight_nodes_.decoder_position_embeddings_weight = gb->mmap_weights(model_folder_path_ + "/decoder_position_embeddings.bias");
    ...
    for (uint32_t i = 0; i < config_.num_layers; i++) {
        ...
        layer.decoder_encoder_attn_output_weight = gb->mmap_embeddings(layer_prefix + "encoder_attn_output.weights");
        ...
        layer.decoder_encoder_attn_output_bias = gb->mmap_embeddings(layer_prefix + "encoder_attn_output.bias");

        layer.decoder_post_encoder_layernorm_weight = gb->mmap_embeddings(layer_prefix + "encoder_attn_norm.weights");
        layer.decoder_post_encoder_layernorm_bias = gb->mmap_embeddings(layer_prefix + "encoder_attn_norm.bias");

        layer.decoder_ffn1_weight = gb->mmap_embeddings(layer_prefix + "mlp_fc1.weights");
        layer.decoder_ffn1_bias = gb->mmap_embeddings(layer_prefix + "mlp_fc1.bias");
        layer.decoder_ffn2_weight = gb->mmap_embeddings(layer_prefix + "mlp_fc2.weights");
        layer.decoder_ffn2_bias = gb->mmap_embeddings(layer_prefix + "mlp_fc2.bias");

        layer.decoder_post_ffn_layernorm_weight = gb->mmap_embeddings(layer_prefix + "final_norm.weights");
        layer.decoder_post_ffn_layernorm_bias = gb->mmap_embeddings(layer_prefix + "final_norm.bias");

        layer.decoder_self_attn_k_weight = gb->mmap_embeddings(layer_prefix + "self_attn_k.wieghts");
        layer.decoder_self_attn_q_weight = gb->mmap_embeddings(layer_prefix + "self_attn_q.wieghts");
        layer.decoder_self_attn_v_weight = gb->mmap_embeddings(layer_prefix + "self_attn_v.wieghts");
        layer.decoder_self_attn_output_weight = gb->mmap_embeddings(layer_prefix + "self_attn_output.wieghts");
        layer.decoder_self_attn_q_bias = gb->mmap_embeddings(layer_prefix + "self_attn_q.bias");
        layer.decoder_self_attn_v_bias = gb->mmap_embeddings(layer_prefix + "self_attn_v.bias");
        layer.decoder_self_attn_output_bias = gb->mmap_embeddings(layer_prefix + "self_attn_output.bias");

        layer.decoder_post_attn_layernorm_weight = gb->mmap_embeddings(layer_prefix + "self_attn_norm.weights");
        layer.decoder_post_attn_layernorm_bias = gb->mmap_embeddings(layer_prefix + "self_attn_norm.bias");

        //Encoder Layers
        std::string layer_prefix = model_folder_path_ + "/encoder.layer_" + std::to_string(i) + "_";

        layer.encoder_ffn1_weight = gb->mmap_embeddings(layer_prefix + "mlp_fc1.weights");
        layer.encoder_ffn1_bias = gb->mmap_embeddings(layer_prefix + "mlp_fc1.bias");
        layer.encoder_ffn2_weight = gb->mmap_embeddings(layer_prefix + "mlp_fc2.weights");
        layer.encoder_ffn2_bias = gb->mmap_embeddings(layer_prefix + "mlp_fc2.bias");

        layer.encoder_post_ffn_layernorm_weight = gb->mmap_embeddings(layer_prefix + "final_norm.weights");
        layer.encoder_post_ffn_layernorm_bias = gb->mmap_embeddings(layer_prefix + "final_norm.bias");

        layer.encoder_self_attn_k_weight = gb->mmap_embeddings(layer_prefix + "self_attn_k.wieghts");
        layer.encoder_self_attn_q_weight = gb->mmap_embeddings(layer_prefix + "self_attn_q.wieghts");
        layer.encoder_self_attn_v_weight = gb->mmap_embeddings(layer_prefix + "self_attn_v.wieghts");
        layer.encoder_self_attn_output_weight = gb->mmap_embeddings(layer_prefix + "self_attn_output.wieghts");
        layer.encoder_self_attn_q_bias = gb->mmap_embeddings(layer_prefix + "self_attn_q.bias");
        layer.encoder_self_attn_v_bias = gb->mmap_embeddings(layer_prefix + "self_attn_v.bias");
        layer.encoder_self_attn_output_bias = gb->mmap_embeddings(layer_prefix + "self_attn_output.bias");

        layer.encoder_post_attn_layernorm_weight = gb->mmap_embeddings(layer_prefix + "self_attn_norm.weights");
        layer.encoder_post_attn_layernorm_bias = gb->mmap_embeddings(layer_prefix + "self_attn_norm.bias");
    }
}

size_t WhisperModel::build_encoder_mlp(CactusGraph* gb, size_t input, uint32_t layer_idx,
...
size_t WhisperModel::build_encoder_attention(CactusGraph* gb, size_t input, uint32_t layer_idx, ComputeBackend backend, bool use_cache = false, size_t position_offset = 0){
    const auto& layer = weight_nodes_.layers[layer_idx];
    ...
    size_t attn = gb->attention(q, k, v, attention_scale_, position_offset = 0);
...
size_t WhisperModel::build_decoder_self_attention(CactusGraph* gb, size_t input, uint32_t layer_idx,
...
size_t WhisperModel::build_encoder_self_attention(CactusGraph* gb, size_t input, uint32_t layer_idx, ComputeBackend backend, bool use_cache = false, size_t position_offset = 0){
    const auto& layer = weight_nodes_.layers[layer_idx];
...
size_t WhisperModel::build_encoder_transformer_block(CactusGraph* gb, size_t hidden, uint32_t layer_idx, ComputeBackend backend, bool use_cache = false, size_t position_offset = 0){
    const auto& layer = weight_nodes_.layers[layer_idx];
...
size_t WhisperModel::build_decoder_transformer_block(CactusGraph* gb, size_t hidden, uint32_t layer_idx, ComputeBackend backend, bool use_cache = false, size_t position_offset = 0){
    const auto& layer = weight_nodes_.layers[layer_idx];
    ...
    size_t ffn_out = build_encoder_mlp(gb, ln3, layer_idx, backend);
...
size_t WhisperModel::forward(const std::vector<uint32_t>& mel_bins,
                             ...
                             bool use_cache)
{
    ...
    size_t pos = gb->slice(weight_nodes_.encoder_position_embeddings, 0, 0, T_enc);
    enc_hidden = gb->add(enc_hidden, pos);
    ...
    size_t pos = gb->slice(weight_nodes_.decoder_position_embeddings_weight, 0, position_offset, new_tokens);
    dec_hidden = gb->add(dec_hidden, pos);
