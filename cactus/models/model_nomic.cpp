...
size_t NomicModel::build_attention(CactusGraph* gb, size_t normalized_input, uint32_t layer_idx,
    ...
    auto attn_output_4d = gb->attention(q_proj_4d, k_proj_4d, v_proj_4d, attention_scale_, false);  // is_causal=false for bidirectional attention
...
size_t NomicModel::build_moe_mlp(CactusGraph* gb, size_t normalized_h, uint32_t layer_idx,
    ...
    auto expert_token_weights_matrix = gb->scatter_topk(topk_idx, topk_w, num_experts);  // [E, N]

    // Getting expert outputs
    auto expert_outputs = 0;  // -> [N, E, D_h]
    for (size_t e = 0; e < num_experts; e++) {
        auto new_expert_output = gb->matmul(normalized_h, layer.mlp_experts_mlp1_weight[e], true, backend);  // [N, D_h] @ [D_e, D_h]^T = [N, D_e]
        new_expert_output = gb->gelu(new_expert_output);
        new_expert_output = gb->matmul(new_expert_output, layer.mlp_experts_mlp2_weight[e], true, backend);  // [N, D_e] @ [D_h, D_e]^T = [N, D_h]

        auto expert_token_weights = gb->index(expert_token_weights_matrix, e, 0);  // [N]
