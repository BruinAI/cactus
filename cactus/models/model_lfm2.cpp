...
size_t LFM2Model::build_conv1d(CactusGraph* gb, size_t input, uint32_t layer_idx,
    ...
    capture_debug_node(layer_idx, "conv_Cg", Cg);
    X  = gb->reshape(X,  {L, C});
    ...
    capture_debug_node(layer_idx, "conv_input_with_cache", conv_input_lc);
    ...
    capture_debug_node(layer_idx, "conv_x_nlc", x_nlc);
    

    const size_t dilation = 1;
    
    size_t y_nlc = gb->conv1d_causal(x_nlc, conv_w, K, dilation); 
    capture_debug_node(layer_idx, "conv_y_nlc", y_nlc);
    
    size_t start = total_len > L ? total_len - L : 0;
    size_t y_slice = gb->slice(y_nlc, 1, start, L);
    capture_debug_node(layer_idx, "conv_y_slice", y_slice);
    size_t y_lc = gb->reshape(y_slice, {L, C});
    capture_debug_node(layer_idx, "conv_y_lc", y_lc);
    
    size_t gated = gb->multiply(Cg, y_lc); 
    capture_debug_node(layer_idx, "conv_gated", gated);
    
    size_t projected = gb->matmul(gated, layer.conv_out_proj_weight, true, backend); 
    capture_debug_node(layer_idx, "conv_projected", projected);
...
size_t LFM2Model::build_attention(CactusGraph* gb, size_t normalized_input, uint32_t layer_idx,
    ...
    capture_debug_node(layer_idx, "attn_q_reshaped", q_proj_reshaped);
    auto q_proj_norm = gb->rms_norm(q_proj_reshaped, layer.attn_q_norm_weight, config_.layer_norm_eps);
    capture_debug_node(layer_idx, "attn_q_norm", q_proj_norm);
    auto q_proj = gb->reshape(q_proj_norm, {batch_seq, num_heads * head_dim});
    capture_debug_node(layer_idx, "attn_q_flat", q_proj);
    ...
    if (use_cache && !kv_cache_.is_empty()) {
        ...
        if (k_view.ptr2 == nullptr && v_view.ptr2 == nullptr) {
            ...
            capture_debug_node(layer_idx, "attn_cache_k_segment", cache_k_node);
            capture_debug_node(layer_idx, "attn_cache_v_segment", cache_v_node);

            final_k = gb->concat(cache_k_node, k_proj_4d, 1);
            final_v = gb->concat(cache_v_node, v_proj_4d, 1);

            capture_debug_node(layer_idx, "attn_k_with_cache", final_k);
            capture_debug_node(layer_idx, "attn_v_with_cache", final_v);
            ...
            capture_debug_node(layer_idx, "attn_cache_k_full", cache_k_node);
            capture_debug_node(layer_idx, "attn_cache_v_full", cache_v_node);
    ...
    capture_debug_node(layer_idx, "attn_k_final", final_k);
    capture_debug_node(layer_idx, "attn_v_final", final_v);

    auto attn_output_4d = gb->attention(q_proj_4d, final_k, final_v, attention_scale_, position_offset);
    capture_debug_node(layer_idx, "attn_scores", attn_output_4d);
    ...
    capture_debug_node(layer_idx, "attn_projected", projected);
...
size_t LFM2Model::build_mlp(CactusGraph* gb, size_t normalized_h, uint32_t layer_idx, ComputeBackend backend) const {
    ...
    capture_debug_node(layer_idx, "mlp_gate_linear", gate);
    
    auto up = gb->matmul(normalized_h, layer.ffn_up_weight, true, backend);
    capture_debug_node(layer_idx, "mlp_up_linear", up);
    
    auto activated = gb->multiply(gb->silu(gate), up);
    capture_debug_node(layer_idx, "mlp_activation", activated);

    auto down = gb->matmul(activated, layer.ffn_down_weight, true, backend);
    capture_debug_node(layer_idx, "mlp_down_linear", down);
...
size_t LFM2Model::build_transformer_block(CactusGraph* gb, size_t hidden, uint32_t layer_idx,
    ...
    capture_debug_node(layer_idx, "block_input", hidden);
    auto normalized_input = gb->rms_norm(hidden, layer.input_layernorm_weight, config_.layer_norm_eps);
    capture_debug_node(layer_idx, "block_input_norm", normalized_input);
    ...
    if (layer_entry.type == WeightNodeIDs::LayerType::CONV) {
        ...
        capture_debug_node(layer_idx, "block_conv_output", block_output);
        ...
        capture_debug_node(layer_idx, "block_attention_output", block_output);
        
    }
    
    auto after_block = gb->add(hidden, block_output);
    capture_debug_node(layer_idx, "block_residual_add", after_block);
    
    auto normalized_after_block = gb->rms_norm(after_block, layer.post_attention_layernorm_weight, config_.layer_norm_eps);
    capture_debug_node(layer_idx, "block_post_attention_norm", normalized_after_block);
    
    auto mlp_output = build_mlp(gb, normalized_after_block, layer_idx, backend);
    capture_debug_node(layer_idx, "block_mlp_output", mlp_output);
...
size_t LFM2Model::forward(CactusGraph* gb, size_t input_embeddings, size_t seq_len,
    ...
    capture_debug_node(static_cast<uint32_t>(config_.num_layers), "final_hidden_pre_norm", hidden);
    auto final_hidden = gb->rms_norm(hidden, weight_nodes_.output_norm_weight, config_.layer_norm_eps);
    capture_debug_node(static_cast<uint32_t>(config_.num_layers), "final_hidden", final_hidden);
...
size_t LFM2Model::forward(CactusGraph* gb, const std::vector<uint32_t>& tokens, 
    ...
    capture_debug_node(0, "embedding_output", hidden);
