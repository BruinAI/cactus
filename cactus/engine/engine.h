...
class LFM2Tokenizer : public Tokenizer {
public:
    LFM2Tokenizer();

    bool load_vocabulary_with_config(const std::string& tokenizer_json_file, const std::string& /*unused*/, const std::string& config_file) override;
...
private:
    ...
    struct Segment {
        ...
        uint32_t id = 0;
        std::string text;
        bool is_first_plain = false;
    };

    uint32_t vocab_size_ = 0;
    uint32_t unk_token_id_ = 0;
    ...
    std::vector<SpecialTokenInfo> special_tokens_;
    std::vector<const SpecialTokenInfo*> special_tokens_sorted_;
    std::unordered_map<uint32_t, SpecialTokenInfo> special_tokens_by_id_;

    mutable std::unordered_map<uint8_t, std::string> byte_to_unicode_;
    mutable std::unordered_map<std::string, uint8_t> unicode_to_byte_;

    void init_byte_encoder() const;
    std::string bytes_to_unicode(const std::string& text) const;
    std::string unicode_to_bytes(const std::string& text) const;
    std::vector<std::string> split_unicode_chars(const std::string& text) const;

    std::pair<int, uint32_t> find_best_merge(const std::vector<std::string>& tokens) const;
    std::vector<std::string> apply_bpe(const std::vector<std::string>& tokens) const;

    std::vector<Segment> tokenize_with_specials(const std::string& text) const;
    std::vector<std::string> gpt2_pre_tokenize(const std::string& text, bool is_first_plain) const;
